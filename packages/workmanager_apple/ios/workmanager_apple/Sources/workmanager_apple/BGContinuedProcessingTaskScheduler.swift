...
extension WorkmanagerPlugin {
    ...
    // AdPocket patch: BGContinuedProcessingTask types exist only in the iOS 26 SDK
    // (Xcode 26 / Swift 6.2). Older toolchains get no-op fallbacks.
    #if compiler(>=6.2)
    @available(iOS 26.0, *)
    ...
    private static func scheduleContinuedProcessingTask(
        ...
        logInfo("BGContinuedProcessingTask needs the iOS 26 SDK (Xcode 26); skipped \(uniqueTaskIdentifier)")
    }
    #endif

    /// Handles a continued-processing task when the SDK knows the type.
    @available(iOS 26.0, *)
    static func handleContinuedProcessingIfSupported(identifier: String, task: BGTask) -> Bool {
        #if compiler(>=6.2)
        if let continued = task as? BGContinuedProcessingTask {
            handleBGContinuedProcessingTask(identifier: identifier, task: continued)
            return true
        }
        #endif
        return false
